...
void b2World::DrawJointReaction(b2Joint* joint)
{
	...
	b2Vec2 f = joint->GetReactionForce(m_debugDraw->GetInvDt());
	float32 m = joint->GetReactionTorque(m_debugDraw->GetInvDt());
	float32 cs = m_debugDraw->GetMomentScale() / 1.e9f*m;
	b2Vec2 p2 = p1 + m_debugDraw->GetForceScale()/1.e9f*f;
	b2Color color(1, 1, 0);
	m_debugDraw->DrawSegment(p1, p2, color);
	if (m>0){ // positive moment in green
		color.Set(0, 1, 0);
	}
	m_debugDraw->DrawCircle(p1, cs, color);
	...
	case e_frictionJoint:
	{
		...
		color.Set(1, 0, 0);
		p2 = p1 + m_debugDraw->GetForceScale() / 1.e9f*rf;
		m_debugDraw->DrawSegment(p1, p2, color);
		cs = m_debugDraw->GetMomentScale() / 1.e9f*ep->GetMaxTorque();
		m_debugDraw->DrawCircle(p1, cs, color);
	}
		break;
