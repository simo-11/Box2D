...
float b2EpJoint_GetReferenceAngle( b2JointId jointId )
{
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	return joint->epJoint.referenceAngle;
}

void b2EpJoint_SetReferenceAngle( b2JointId jointId, float angleInRadians )
{
	B2_ASSERT( b2IsValidFloat( angleInRadians ) );
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	joint->epJoint.referenceAngle = b2ClampFloat(angleInRadians, -B2_PI, B2_PI);
}

void b2EpJoint_SetLinearHertz( b2JointId jointId, float hertz )
{
	B2_ASSERT( b2IsValidFloat( hertz ) && hertz >= 0.0f );
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	joint->epJoint.linearHertz = hertz;
}

float b2EpJoint_GetLinearHertz( b2JointId jointId )
{
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	return joint->epJoint.linearHertz;
}

void b2EpJoint_SetLinearDampingRatio( b2JointId jointId, float dampingRatio )
{
	B2_ASSERT( b2IsValidFloat( dampingRatio ) && dampingRatio >= 0.0f );
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	joint->epJoint.linearDampingRatio = dampingRatio;
}

float b2EpJoint_GetLinearDampingRatio( b2JointId jointId )
{
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	return joint->epJoint.linearDampingRatio;
}

void b2EpJoint_SetAngularHertz( b2JointId jointId, float hertz )
{
	B2_ASSERT( b2IsValidFloat( hertz ) && hertz >= 0.0f );
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	joint->epJoint.angularHertz = hertz;
}

float b2EpJoint_GetAngularHertz( b2JointId jointId )
{
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	return joint->epJoint.angularHertz;
}

void b2EpJoint_SetAngularDampingRatio( b2JointId jointId, float dampingRatio )
{
	B2_ASSERT( b2IsValidFloat( dampingRatio ) && dampingRatio >= 0.0f );
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	joint->epJoint.angularDampingRatio = dampingRatio;
}

float b2EpJoint_GetAngularDampingRatio( b2JointId jointId )
{
	b2JointSim* joint = b2GetJointSimCheckType( jointId, b2_epJoint );
	return joint->epJoint.angularDampingRatio;
}

b2Vec2 b2GetEpJointForce( b2World* world, b2JointSim* base )
{
	b2Vec2 force = b2MulSV( world->inv_h, base->epJoint.linearImpulse );
	return force;
}

float b2GetEpJointTorque( b2World* world, b2JointSim* base )
{
	return world->inv_h * base->epJoint.angularImpulse;
}
...
void b2PrepareEpJoint( b2JointSim* base, b2StepContext* context )
{
	B2_ASSERT( base->type == b2_epJoint );
...
void b2WarmStartEpJoint( b2JointSim* base, b2StepContext* context )
{
...
void b2SolveEpJoint( b2JointSim* base, b2StepContext* context, bool useBias )
{
	B2_ASSERT( base->type == b2_epJoint );
