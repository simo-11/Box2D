...
void Frame::build() {
	...
	bd.angle = b2_pi / 2;
	float sx;
	float step = b2Frame::so_count * 2 * b2Frame::hx;
	...
	{ // rescale using m_zoom and m_center if dimensions change
		float cx = 2 * b2Frame::so_count*b2Frame::hx;
		float cy = sy / 2 + b2Frame::hy + 1;
		if (b2Frame::lastCy != cy || b2Frame::lastCx != cx) {
			float margin = 2 * b2Frame::hy + 2;
