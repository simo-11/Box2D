...
struct Settings
{
	...
	b2Vec2 addMassPoint;
	float addMass = 1000000, addMassSize=2;
	bool initImpulses;
	float forceScale, momentScale;
	float epbMaxForce, epbMaxMoment, epbX, epbY,
		epbMass, epbHz, epbMaxRotation, epbMaxStrain, epbMaxElasticRotation;
	bool epbDebugListener;
	float gravityRampUpTime;
	float bombMass, bombWidth, bombMultiplier;
	BombShape bombShape;
...
public:
	...
	void LogJoint(b2Joint* j, float fScale, float mScale, float[4],
		const char * fmt = "%5.2f", float maxValue = (float)FLT_MAX, float minMaxValue = 0.f);
	void LogSelectedJoints(float fScale, float mScale, float[4],
		const char * fmt = "%5.2f", float maxValue = (float)FLT_MAX, float minMaxValue = 0.f);
